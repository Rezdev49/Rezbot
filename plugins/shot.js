...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    if (m.chat in conn.casino) return m.reply ('Masih ada yang melakukan shot disini, tunggu sampai selesai!!')
    ...
    try {
        ...
        if (global.db.data.users[m.sender].exp >= count * 1) {
            ...
            if (Aku > Kamu) {
                conn.reply(m.chat, `*SHOT GAMES*\n*Tembakan kamu 🎯:* ${Kamu} Point\n*Tembakan computer 🎯:* ${Aku} Point\n\n*You LOSE*\nKamu kehilangan❎ ${count} Uang(xp)`.trim(), m)
            } else if (Aku < Kamu) {
